...
bonus_share_per_treatment <- bonus_share %>% group_by(treatment, player_unique_ID)%>% 
...
d1 <- bonus_share_per_treatment %>% group_by(treatment) %>% summarise(
...
d2=bonus_share_per_matching %>% group_by(matching) %>% summarise(
  mean_session=mean(perc_share),
  se_session=se(perc_share),
  sd_session=sd(perc_share),
  median_session = median(perc_share),
...
t.test(bonus_share_per_matching$perc_share~bonus_share_per_matching$matching)
...
ggplot(data=d1)+
  geom_bar(aes(x=treatment, y=mean_session), stat="identity", fill="blue", alpha=0.7, width=0.4)+ylab("% of Sharing")+theme_bw()+xlab("Treatments")+
...
ggplot(data=d1 %>% subset.data.frame(subset = treatment %in% c("P2", "S2")), aes(x=treatment, y=mean_session, fill = treatment))+
  geom_bar(stat="identity", fill="blue", alpha=0.7, width=0.4)+
  ylab("% of Sharing")+xlab("Treatments")+geom_col(colour = "black") +
  geom_errorbar(aes(x=treatment, ymin=mean_session-se_session, ymax=mean_session+se_session), width=0.1, colour="black", alpha=0.9, size=1) + theme_light() +
...
ggplot(data=d1 %>% subset.data.frame(subset = treatment %in% c("P4", "S4")), aes(x=treatment, y=mean_session, , fill = treatment))+
  geom_bar(stat="identity", fill="blue", alpha=0.7, width=0.4)+
  ylab("% of Sharing")+xlab("Treatments")+geom_col(colour = "black") + ylim(c(0, 0.8))+
  geom_errorbar(aes(x=treatment, ymin=mean_session-se_session, ymax=mean_session+se_session), width=0.1, colour="black", alpha=0.9, size=1) + theme_light() +
...
wilcox.test(wilcox_test1$player.share_decision~wilcox_test1$treatment)
## Stranger
...
wilcox.test(first_round_sharing_S$player.share_decision~first_round_sharing_S$treatment)
## effect of matching
## K = 2
first_round_sharing_K2 <- bonus_share_longitudinal %>% 
  subset.data.frame(subset = subsession.round_number==1 
                    & (treatment=="P2" | treatment=="S2"))
wilcox.test(first_round_sharing_K2$player.share_decision~first_round_sharing_K2$treatment)
...
wilcox.test(first_round_sharing_K4$player.share_decision~first_round_sharing_K4$treatment)
...
fifteen_round_sharing <- bonus_share_longitudinal %>% 
  subset.data.frame(subset = subsession.round_number<16)
...
fifteen_round_sharing_K2 <- bonus_share_longitudinal_K2 %>% subset.data.frame(subset = subsession.round_number==15)
...
second_half_sharing <- bonus_share_longitudinal_treatment %>%  subset.data.frame(subset = subsession.round_number>0)
data.frame(
...
time_2 <- ggplot(data=plot_1_data, aes(x=subsession.round_number, y=perc_share))+
  ...
  ylim(c(min(plot_1_data$perc_share),1))+theme_classic()+xlab("Round")+ylab("% of Sharing")+
...
time_4 <- ggplot(data=plot_2_data, aes(x=subsession.round_number, y=perc_share))+
  ...
  ylim(c(0.4,1))+theme_classic()+xlab("Round")+ylab("% of Sharing")+
...
model0 <- glmer(player.share_decision~partner + (1|session.code) + (1|id_actor), data = bonus_share_longitudinal,family=binomial(link="logit"))
model0.1 <- glmer(player.share_decision~partner + subsession.round_number + (1|session.code) + (1|id_actor), data = bonus_share_longitudinal,family=binomial(link="logit"))
model0.2 <- glmer(player.share_decision~partner + subsession.round_number*partner + (1|session.code) + (1|id_actor), data = bonus_share_longitudinal,family=binomial(link="logit"))
...
model1 <- glmer(player.share_decision~treatment + (1|session.code) + (1|id_actor), data = bonus_share_longitudinal,family=binomial)
model1.1 <- glmer(player.share_decision~partner + partner:K + K + (1|id_actor), data = bonus_share_longitudinal,family=binomial)
...
model3 <- glmer(player.share_decision~treatment + subsession.round_number + (1|session.code)+ (1|id_actor), data = bonus_share_longitudinal,family=binomial)
model3.1 <- glmer(player.share_decision~treatment + subsession.round_number*treatment + (1|session.code)+ (1|id_actor), data = bonus_share_longitudinal, family=binomial)
# n. decision of individual instead of round (robustness)
model3.2 <- glmer(player.share_decision~treatment + n_innovations_found + (1|session.code) + (1|id_actor), data = bonus_share_longitudinal,family=binomial(link="logit"))

# simulate posteriors
sim_model1 <- coef(sim(model1, data=bonus_share_longitudinal))
